...
and the chain data are lost after you shut down the docker container. Before
doing so, place the correct path to the *battery-station-relay.json* file 
and select a custom node name.
...
Assuming the path you want to use locally is */services/zeitgeist* (and that
docker has write permissions in this folder), 
...
desire for chain data and that docker has write permissions for it.
Replace `zeitgeist-support-$RANDOM` with the name your node should have.

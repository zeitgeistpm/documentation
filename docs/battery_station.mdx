...
offer_ _a
[pre-built binary](https://github.com/zeitgeistpm/zeitgeist/releases/download/v0.2.1/zeitgeist_parachain)._
...
```bash
curl -o zeitgeist https://github.com/zeitgeistpm/zeitgeist/releases/download/v0.2.1/zeitgeist_parachain
chmod +x zeitgeist
```
...
```bash
# clone the code locally and checkout version v0.2.1
git clone https://github.com/zeitgeistpm/zeitgeist.git --depth 1 --branch v0.2.1
# change directory
cd zeitgeist
```
...
accept incoming traffic. If you change one of those, the changes must be 
reflected in the option `-p CHANGE_THIS:port`.

...
offer_ _a
[pre-built binary](https://github.com/zeitgeistpm/zeitgeist/releases/download/v0.2.2/zeitgeist_parachain)._
...
```bash
curl -o zeitgeist https://github.com/zeitgeistpm/zeitgeist/releases/download/v0.2.2/zeitgeist_parachain
chmod +x zeitgeist
```
...
```bash
# clone the code locally and checkout version v0.2.2
git clone https://github.com/zeitgeistpm/zeitgeist.git --depth 1 --branch v0.2.2
# change directory
cd zeitgeist
```
...
Create a folder that will contain the Zeitgeist data and the previously compiled binary
file. Note where your _zeitgeist_ binary lies (by default it is within the source folder at
...
_Note: The arguments after the double dash (--) apply to the relay chain,
which is required by the parachain_
...
Set `--port`, `--rpc-port` and `--ws-port` to some ports that are not 
occupied yet. Ensure that incoming traffic on those ports is allowed. 
The node will listen to RPCs over HTTP and WS, to make the node
...
```bash
docker pull zeitgeistpm/zeitgeist-node-parachain:v0.2.2
```
...
docker run \
    ...
    --restart=always \
    zeitgeistpm/zeitgeist-node-parachain:v0.2.2 \
...
and WS, to make the node eligible for potential future load-balancing. Ensure that you
...
with the name your node should have.
...
```bash
docker ps --filter ancestor=zeitgeistpm/zeitgeist-node-parachain:v0.2.2
```
